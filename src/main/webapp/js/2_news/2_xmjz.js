        ...
        success:function (data){
            ...
            if (data) {
            	var res = '';
	        		...
	        		res += 		"</p>";
	        		res += 		"<a class='body1_in_left_a' href='index_xmjz_xq.html?id="+(data[0].id!=undefined?data[0].id:'')+"'><span>查看详情</span></a>"
	        		res += "</div>";
	        		//图片
...
//	}
	...
	$.ajax({
        ...
        async : false, 
        data:{menuId:'8aad12b1533058a80153318e6e97000e',
        	pageCurrent:p,eachPageSize:5,
        	isCalFirst:"F"	//不从第0条数据开始-标识符
        		},
        success:function (data){
        	if(flag){
        		//清除数据
            ...
            if (data) {
            	...
            	if(data.length==0){
            		$('.body2_ckgd').remove();//没有数据了，故将“产看更多”移除掉
            	}
            	...
            	var res = '';
            		for(var i=0;i<data.length;i++){
    	        		...
    	        		res += "</div>";
    	        		//内容
    	        		res += "<a href='index_xmjz_xq.html?id="+(data[i].id!=undefined?data[i].id:'')+"' class='body2_li_r'>";
